  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl lg:text-[28px] font-bold text-foreground">Recall de Pacientes</h1>
          <div className="flex items-center gap-3 mt-1">
            <p className="text-sm text-muted-foreground">{recalls.length} recalls cadastrados</p>
            {overdueCount > 0 && (
        ...
        {FILTER_OPTIONS.map((opt) => {
          ...
          return (
            <button
              ...
              style={statusFilter === opt.value ? { backgroundColor: 'var(--primary)' } : {}}
            >
              {opt.label}
              {count > 0 && (
            ...
            return (
              <div
                ...
                className={`bg-card rounded-xl border p-4 flex items-center gap-4 transition-all ${
                  overdue ? 'border-amber-300 bg-amber-50/30' : 'border-border'
                }`}
              >
                {/* Alerta vencido */}
                {overdue && (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="font-medium text-foreground text-sm">{recall.lead.name}</p>
                    <span
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {recall.status !== 'done' && recall.status !== 'cancelled' && (
                    <div className="relative group">
                      ...
                      <div className="absolute right-0 top-9 bg-card border border-border rounded-lg shadow-lg py-1 w-36 hidden group-focus-within:block z-10">
                        <button
                          type="button"
                          className="w-full text-left px-3 py-2 text-xs text-muted-foreground hover:bg-muted/60"
