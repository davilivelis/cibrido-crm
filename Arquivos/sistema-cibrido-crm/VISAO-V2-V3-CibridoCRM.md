...
- **Painel Administrativo Cíbrido (face interna):**
  - Visão de TODAS as clínicas (lista com status: ativa, trial, inadimplente, cancelada)
  - Métricas consolidadas (total de leads, total de clínicas, MRR, churn)
  - Acessar dados de qualquer clínica (como síndico com chave mestra)
  - Exportar dados de qualquer clínica (CSV/ZIP) pra entrega ou backup
  - Painel separado do CRM do cliente (rota /admin, acesso só Cíbrido)
...
- **Sistema de backup/archive pós-cancelamento:**
  - Quando cliente cancela (dia 90): dados saem da view ativa
  - Dados movidos pra tabela de archive (não deletados)
  - Exportação enviada pro cliente (direito dele)
  - Cíbrido mantém cópia no archive permanentemente
  - Dados do archive acessíveis pelo Painel Admin Cíbrido
  - Útil pra: análise histórica, compliance, reativação futura
...
| Cliente cancela e pede dados | Exportação automática + archive Cíbrido |
| Ex-cliente processa por dados | Archive com cópia + log de entrega |
